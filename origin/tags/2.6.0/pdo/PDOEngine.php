		...
		switch ($this->dbType){
			case 'sqlite':
				...
				if (is_file(FQDB)){
					...
					if (!$s){
						$wpdb->bail(print_r($this->pdo->errorInfo(), true));
					}
...
HTML;
	...
	private function lock($type='lock'){
		
		if ($this->dbType = 'sqlite'){
			$file = FQDBDIR . '/lockFile';
			switch ($type):
				case 'lock':
	...
	*/
	...
	private function executeQuery( $statement ){
		global $wpdb;
		file_put_contents(ABSPATH . '/wp-content/database/debug2.txt', print_r($statement, true), FILE_APPEND);
		file_put_contents(ABSPATH . '/wp-content/database/debug2.txt', print_r($this->extractedVariables, true), FILE_APPEND);
		...
		file_put_contents(ABSPATH . '/wp-content/database/debug2.txt', print_r($statement->errorInfo(),true), FILE_APPEND);
		if ($result === false){
