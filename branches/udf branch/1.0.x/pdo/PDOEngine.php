...
class PDO_Engine{
	 ...
	 */
	public function PDO_Engine($connectionParams){
	...
	private function connect($connectionParams){
		...
		switch ($this->dbType){
			case 'sqlite':
				...
				//create the sqlite udfs
				require_once ABSPATH . '/wp-content/pdo/driver_sqlite/sqlite_udfs.php';
...
HTML;
	...
	*/
	private function prepareQuery(){
		...
		$statement = $this->pdo->prepare($this->preparedQuery);
		if ($statement === false){
			$message = "Problem preparing the PDO SQL Statement.  Error was ".print_r($this->pdo->errorInfo(), true);
			$this->setError(__LINE__, __FUNCTION__, $message);
			return false;
		}else{
	...
	private function executeQuery( $statement ){
		...
		if ($result === false){
			$message = "Error executing query. Error was was ".print_r($statement->errorInfo(), true);
			$this->setError(__LINE__, __FUNCTION__, $message);
			return FALSE;
		}else {
...
/**
 * helper function to dump debug information to a file
 */
function __d($output){
	file_put_contents(ABSPATH .'/wp-content/database/debug2.txt', $output . "\r\n", FILE_APPEND);
}
?>