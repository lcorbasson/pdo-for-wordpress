...
/**
 * working notes
 * LIKE to become ILIKE
 * backticks to be stripped
 * fix autoincrements
 * 
 * 
 */

class pdo_postgres_driver{
	...
	*/
	private function _rewrite_datetime_functions($matches){
		...
		switch (strtolower($matches[1])){
			...
			break;
			case "month":
				return " strftime('%m',{$matches[2]}) ";
			break;
			case "day":
			case "dayofmonth":
				return " strftime('%d',{$matches[2]}) ";
			break;
			case "unix_timestamp":
				return "strftime('%s', {$matches[2]}) ";
			break;
		}
	}
