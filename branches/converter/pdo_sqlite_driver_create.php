...
class createQuery{
	 ...
	 */
	public function rewriteQuery($query, $rtn='array'){
		$this->_query[] = $query;
		$this->_errors [] = '';
		$this->stripBackticks();
		$this->getTableName();
	...
	private function getTableName(){
		$pattern = '/^\s*CREATE\s*(?:TEMP|TEMPORARY)?\s*TABLE\s*(?:IF NOT EXISTS)?\s*(?P<tName>[^\(]*)/imsx';
		preg_match($pattern, $this->getCurrentQuery(), $matches);
		$tName = str_ireplace('if not exists', '', $matches['tName']);
	...
	/**
	 *	method for stripping the comments from the create query
	 *
	...
	private function _rewriteKey($matches){
		...
		$this->indexQueries[] = "CREATE INDEX {$field}_{$r} on " . $this->tableName . "(".$matches[2] .")";
		return '';
	}
	 ...
	 */
	private function postProcess($rtn = 'array'){
	...
	private function rewrite_character_set_(){
		$pattern = '/\b(default)?\s*character\s*set\s*(?<!\()[^ ]*/im';
		$this->addQuery(preg_replace($pattern, ' ', $this->getCurrentQuery()));
		$pattern = '/\b(default)?\s*charset\s?=\s?[a-z][^\s]*/im';
		$this->addQuery(preg_replace($pattern, ' ' , $this->getCurrentQuery()));
	}
}